...
public class BytesWritable extends BinaryComparable
  ...
  /**
   * Get the raw backing array without adjusting payload offset.
   *
  ...
  public void expandIfNecessary(int newSize) {
    if (offset != 0) {
      throw new IllegalStateException("expandIfNecessary requires offset=0");
    }
  ...
  public void setCapacity(final int capacity) {
    if (capacity != getCapacity()) {
      ...
      if (offset == 0) {
        ...
        if (size > 0) {
          System.arraycopy(this.bytes, offset, newBytes, 0, this.size);
        }
        this.bytes = newBytes;
        this.offset = 0;
      }
  ...
  public String toString() {
    return IntStream.range(0, size)
        .mapToObj(idx -> String.format("%02x", bytes[offset + idx]))

...
public class FetcherUnordered extends Fetcher<FetchedInput> {
  ...
  public FetcherUnordered createClone() {
    return new FetcherUnordered(
        ...
        fetcherConfigCommon, fetcherConfig, taskContext, this.attempt + 1, shuffleManager);
  }
  ...
  private CompositeInputAttemptIdentifier[] fetchInputs(
    ...
    try {
      ...
      for (MapOutputStat mapOutputStat : mapOutputStats) {
        ...
        if (fetchedInput.getType() == ShuffleClient.Type.MEMORY) {
          ...
          throw new TezUncheckedException("Cannot fetch unordered shuffle input " + srcAttemptId
              + " from " + host + ": expected MEMORY or DISK output, but allocator returned "
              + fetchedInput.getType());
        }
      ...
      LOG.warn("{}: Failed to shuffle output of {} from {} to {}",
          logIdentifier, srcAttemptId, host, fetcherConfigCommon.localHostName, ioe);

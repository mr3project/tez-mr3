  ...
  private void fetchMultipleBlocks() throws IOException {
    ...
    try {
      while (totalReceivedBytes < blockLength) {
        ...
        // Mapper because:
        //   1) Celeborn does not order Mapper output blocks with their indexes.
        //   2) a single Mapper may produce multiple blocks.
        // Still this is safe for our purpose because we only have to process blockLength bytes.
        ...
        InputAttemptIdentifier identifierForCurrentBlock = new InputAttemptIdentifier(
            firstInputAttemptIdentifier.getInputIdentifier(),
            firstInputAttemptIdentifier.getAttemptNumber(),
            firstInputAttemptIdentifier.getPathComponent(),
            false,
        ...
        MapOutput mapOutput = allocator.reserve(identifierForCurrentBlock, actualSize, actualSize, fetcherId, true);
        ...
        // treat all blocks as output corresponding to firstInputAttemptIdentifier
        // calling copySucceeded() for each block is necessary for mapOutput.commit() etc
        long copyDuration = System.currentTimeMillis() - startTime;
        shuffleScheduler.copySucceeded(identifierForCurrentBlock, null, dataLength, dataLength, copyDuration, mapOutput, false);
      }
    } finally {
      closeRssShuffleInputStream();
    }
    ...
    // except for firstInputAttemptIdentifier, treat all inputAttemptIdentifiers as reading empty blocks

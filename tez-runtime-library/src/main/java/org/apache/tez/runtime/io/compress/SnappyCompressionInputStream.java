...
final class SnappyCompressionInputStream extends InputStream {
  ...
  SnappyCompressionInputStream(
    ...
    try {
      ...
      if (magic != SnappyCompressionOutputStream.MAGIC) {
        throw new IOException("Invalid Snappy stream magic");
      }
    } catch (EOFException e) {
      throw new IOException("Truncated Snappy stream header", e);
    }
  }
  ...
  private void readChunk() throws IOException {
    ...
    try {
      ...
      throw new IOException("Truncated Snappy chunk header", e);
    }
    if (rawLength == 0 && compressedLength == 0) {
      if (input.read() != -1) {
        throw new IOException("Trailing data after Snappy stream terminator");
      }
      ended = true;
      return;
    }
    if (rawLength <= 0 || rawLength > maximumBlockSize) {
      throw new IOException("Invalid Snappy uncompressed chunk length: " + rawLength);
    }
    int maximumCompressed;
    try {
      maximumCompressed = org.xerial.snappy.Snappy.maxCompressedLength(rawLength);
    } catch (RuntimeException e) {
      throw new IOException("Invalid Snappy chunk length: " + rawLength, e);
    }
    ...
    try {
      ...
      throw new IOException("Truncated Snappy chunk payload", e);
    }
    int actual = decompressor.decompressBuffered(compressedLength, rawLength);
    if (actual != rawLength) {
      throw new IOException("Snappy chunk length mismatch: expected " + rawLength + ", decoded " + actual);
    }
    position = 0;
    limit = actual;
  }

...
public class TezMerger {
  ...
  extends PriorityQueue<Segment> implements TezRawKeyValueIterator {
    ...
    private void compareKeyWithNextTopKey() {
      sameKey = false;
    ...
    public boolean next() throws IOException {
      ...
      if (!minSegment.inMemory()) {
        // When we load the value from an in-memory segment,
        // we reset the "value" DIB in this class to the in-mem segment's byte[].
        // When we load the value bytes from disk,
        // we shouldn't use the same byte[] since it would corrupt the data in the in-mem segment.
        // So we maintain an explicit DIB for value bytes obtained from disk,
        // and if the current segment is a disk segment,
        // we reset the "value" DIB to the byte[] in that
        // (so we reuse the disk segment DIB whenever we consider a disk segment).
